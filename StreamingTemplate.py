...
status['SetDataBuffer'] = ps.ps5000aSetDataBuffer(chandle, channel, buffer_data.ctypes.data_as(ctypes.POINTER(ctypes.c_int16)), buffer_size, 0, buffer_mode)
...
cPointerFunc = ps.StreamingReadyType(streaming_callback) # Create a Pointer of our callback function
pParameter = None # Parameters will be passed in the streaming callback function in the 'Parameter' parameter

RUN = True
while RUN:
    wasCalledBack = False
    status['GetStreamingLatestValues'] = ps.ps5000aGetStreamingLatestValues(chandle, cPointerFunc, pParameter)

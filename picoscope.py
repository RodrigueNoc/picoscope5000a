...
class Picoscope():
    ...
    def StartPico(self, serial=None, resolution=1):
        """
        ...
        self.status['OpenUnit'] = ps.ps5000aOpenUnit(
            ctypes.byref(self.handle), self.serial, self.resolution)
        assert_pico_ok(self.status['OpenUnit'])

    def Channel(self, Cha="A", channel_enabled=1, channel_type='DC',
                channel_range="2V", analogueOffset=0.):
        """
        ...
        """
        ps_channel = ps.PS5000A_CHANNEL[f'PS5000A_CHANNEL_{Cha}']
        ps_channel_type = ps.PS5000A_COUPLING[f'PS5000A_{channel_type}']
        channel_enabled = channel_enabled
        ps_channel_range = ps.PS5000A_RANGE[f'PS5000A_{channel_range}']
        analogueOffset = analogueOffset
        self.ps_voie[Cha] = [ps_channel, ps_channel_type, channel_enabled, ps_channel_range, analogueOffset]

        self.status['SetChannel'] = ps.ps5000aSetChannel(self.handle, ps_channel, channel_enabled, ps_channel_type, ps_channel_range, analogueOffset)
    ...
    def MAX_ADC(self):
        ...
        self.status["maximumValue"] = ps.ps5000aMaximumValue(self.handle, ctypes.byref(self.maxADC))
        assert_pico_ok(self.status["maximumValue"])

    def SetTrigger(self, trigger_enabled = 1, Cha = "A", seuil = 1000, direction = "FALLING", trigger_delay = 0, trigger_autoTrigger_ms = 0):
        """
        ...
        """
        self.MAX_ADC()
        trigger_threshold = mV2adc(seuil, self.ps_voie[Cha][3], self.maxADC)
        trigger_direction = ps.PS5000A_THRESHOLD_DIRECTION[f'PS5000A_{direction}']
        
        self.status['SetSimpleTrigger'] = ps.ps5000aSetSimpleTrigger(self.handle, trigger_enabled, self.ps_voie[Cha][0], trigger_threshold, trigger_direction, trigger_delay, trigger_autoTrigger_ms)
        assert_pico_ok(self.status['SetSimpleTrigger'])

    def StartStreaming(self, Cha = "A", buffer_size = 1000, mode = "NONE", Ratio = 1, autoStop = 0, intervale = 1, unite = 'NS'):
        """
        ...
        intervale : donne le temps de l'intervale de mesure (unitée préciser après)
        unite : p54, 4.18.3; unitée d'un intervale de mesure;
            possible values : "FS", "PS", "NS", "US", "MS", "S"
        """
        # Initialisation du stockage
        self.buffer_size = buffer_size
        self.buffer_data = np.zeros(shape=buffer_size, dtype=np.int16)
        self.mode = ps.PS5000A_RATIO_MODE[f'PS5000A_RATIO_MODE_{mode}']
        downSampleRatio = Ratio

        self.status['SetDataBuffer'] = ps.ps5000aSetDataBuffer(self.handle, self.ps_voie[Cha][0], self.buffer_data.ctypes.data_as(ctypes.POINTER(ctypes.c_int16)), self.buffer_size, 0, self.mode)
        ...
        self.status['StartStreaming'] = ps.ps5000aRunStreaming(self.handle, ctypes.byref(self.sampleInterval), self.sampleIntervalTimeUnits, maxPreTriggerSamples, maxPostTriggerSamples, self.autoStop, downSampleRatio, self.mode, self.buffer_size)
